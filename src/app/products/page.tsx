...
const SEARCH_PLACEHOLDERS = [
  "Search for Plugin Versions...",
  "Search for Stem Packs...",
  "Search for Master Projects...",
  "Search for Collaborative Sessions...",
  "Search for Sound Libraries..."
];
...
export default function ProductsPage() {
  ...
  return (
    <div className="bg-[var(--background)] min-h-screen relative selection:bg-[var(--accent)] selection:text-white overflow-x-hidden transition-colors duration-500">
      <Navbar />
            ...
            <div className="relative h-full w-full bg-[#0d2360]">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-center px-6">
                <div className="max-w-4xl mt-24">
                  ...
                  <h2 className="hero-text-animate text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 uppercase tracking-tight leading-[1.1]">Intelligent <br /> DAW Control.</h2>
                  <p className="hero-text-animate text-white/70 text-sm md:text-lg mb-8 max-w-2xl mx-auto font-medium">Precision version control solutions serving world-class producers and architectural sound design.</p>
              ...
              <div className="absolute inset-0 flex items-center justify-center text-center px-6">
                <div className="max-w-4xl mt-24">
                  <div className="hero-text-animate inline-flex items-center gap-2 px-4 py-1 border border-white/30 text-white text-[9px] font-black uppercase tracking-[0.25em] rounded-full mb-6 backdrop-blur-sm">
                    <MdAutoAwesome size={14} /> Global Production 2024
                  </div>
                  <h2 className="hero-text-animate text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 uppercase tracking-tight leading-[1.1]">Legacy In <br /> Every Beat.</h2>
                  <p className="hero-text-animate text-white/90 text-sm md:text-lg mb-8 max-w-2xl mx-auto font-medium">Explore premium project components designed for high-end modern music production.</p>
                  <div className="hero-text-animate">
      ...
      <div className="relative -mt-12 z-30 px-6 md:px-10 lg:px-15 max-w-5xl mx-auto">
        <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-3 px-1 scrollbar-hide">
              <button
                ...
                className={`flex-shrink-0 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shadow-md border
                                    ${activeFilter === 'All' ? 'bg-[var(--foreground)] border-[var(--foreground)] text-[var(--background)]' : 'bg-[var(--surface)] text-[var(--foreground-muted)] border-[var(--border)] hover:border-[var(--accent)]'}`}
              >
                All Products
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`flex-shrink-0 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shadow-md border
                                        ${activeFilter === cat.slug ? 'bg-[var(--foreground)] border-[var(--foreground)] text-[var(--background)]' : 'bg-[var(--surface)] text-[var(--foreground-muted)] border-[var(--border)] hover:border-[var(--accent)]'}`}
                >
                  {cat.name}
                </button>
              ))}
          ...
          <div className="relative">
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-2 shadow-xl flex items-center transition-all group focus-within:border-[var(--accent)]/50">
              <div className="w-10 h-10 flex items-center justify-center text-[var(--foreground-muted)]">
                <MdSearch size={22} />
              </div>
              <input
                type="text"
                placeholder={placeholder}
                className="flex-1 px-2 py-3 text-[14px] font-bold outline-none border-none focus:ring-0 placeholder:text-[var(--foreground-muted)] bg-transparent text-[var(--foreground)]"
              />

              <div className="h-6 w-[1px] bg-[var(--border)] mx-2 hidden md:block" />
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all
                                    ${isFilterOpen ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-[var(--foreground-muted)] hover:text-[var(--accent)]'}`}
              >
            ...
            {searchQuery.length > 0 && (
              <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-4 md:p-6">
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-[9px] font-black text-[var(--accent)] uppercase tracking-widest">Inventory Results</p>
                    <button onClick={() => setSearchQuery('')} className="text-[var(--foreground-muted)] hover:text-red-500 transition-colors">
                      <MdClose size={18} />
                    </button>
                  </div>
                    ...
                    {productsLoading ? (
                      ...
                      allFilteredProducts.slice(0, 8).map((product) => (
                        <div
                          ...
                          className="flex items-center gap-4 p-3 rounded-xl hover:bg-[var(--surface-hover)] cursor-pointer group transition-all"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[8px] font-black text-[var(--foreground-muted)] uppercase tracking-widest mb-0.5">{typeof product.category === 'string' ? product.category : product.category?.name}</p>
                            <h5 className="text-[13px] font-black text-[var(--foreground)] uppercase tracking-tight truncate group-hover:text-[var(--accent)] transition-colors leading-tight">{product.name}</h5>
                            <p className="text-[var(--accent)] text-[11px] font-black tracking-tight mt-1">
                  ...
                  {allFilteredProducts.length > 8 && (
                    <div className="mt-4 pt-4 border-t border-slate-50 text-center">
                      <button
                        ...
                        className="text-[9px] font-black text-[var(--foreground)] uppercase tracking-widest hover:text-[var(--accent)] transition-all flex items-center justify-center gap-2 mx-auto"
                      >
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col mb-10">
              <div className="flex items-center gap-3 mb-2">
                <MdAutoAwesome className="text-[var(--accent)]" size={20} />
                <h3 className="text-[10px] font-black text-[var(--accent)] uppercase tracking-[0.4em]">Signature Series</h3>
              </div>
              <h2 className="text-3xl md:text-4xl font-black text-[var(--foreground)] uppercase tracking-tighter leading-none">Curated Selection.</h2>
            </div>
      ...
      <section className="pt-16 pb-40 px-6 md:px-10 lg:px-15 catalog-grid">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 pb-10 border-b border-[var(--border)]">
            <div>
              <h3 className="text-4xl font-black text-[var(--foreground)] uppercase tracking-tighter leading-none mb-3">The Collection</h3>
              <p className="text-[var(--foreground-muted)] text-[9px] font-black uppercase tracking-[0.2em]">Inventory / {activeFilter} / {allFilteredProducts.length} Results</p>
            </div>
              ...
              {['newest', 'popular', 'price-low', 'price-high'].map(s => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border
                                        ${sortBy === s ? 'bg-[var(--foreground)] border-[var(--foreground)] text-[var(--background)]' : 'bg-transparent border-[var(--border)] text-[var(--foreground-muted)] hover:border-[var(--accent)]'}`}
                >
                  {s.replace('-', ' ')}
                </button>
              ))}
