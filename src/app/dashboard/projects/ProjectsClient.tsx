        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            <div>
                <div className="flex items-center gap-2 text-sm text-gray-400 mb-2">
                    ...
                    <span>/</span>
                    <span className="text-[#00f2ff]">Projects</span>
                </div>
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-black text-white">Projects</h1>
                        <p className="text-gray-500 text-sm font-medium">Manage and track your music projects and version history.</p>
                    </div>
                    <button
                        onClick={() => router.push('/dashboard/projects/new')}
                        className="bg-[#00f2ff] text-black px-6 py-3 rounded-xl font-black text-sm flex items-center gap-2 hover:bg-[#00d8e6] transition-all"
                    >
                ...
                <div className="lg:col-span-3">
                    <div className="bg-[#111e48]/40 border border-white/5 rounded-3xl overflow-hidden min-h-[50vh]">
                        <div className="px-8 py-6 border-b border-white/5 bg-white/5 flex justify-between items-center">
                            ...
                            <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{projects.filter(p => p.has_active_push).length} Active Pushes</span>
                        </div>

                        {projectsLoading ? (
                            <div className="flex flex-col items-center justify-center py-20">
                                <FiLoader className="animate-spin text-[#00f2ff] mb-4" size={48} />
                            ...
                            <div className="divide-y divide-white/5">
                                {projects.map((project) => (
                                    <div key={project.id} className="p-8 hover:bg-white/5 transition-all flex flex-col md:flex-row gap-6 items-center">
                                        <div className="w-14 h-14 bg-white/5 rounded-xl flex items-center justify-center text-[#00f2ff] shrink-0 font-black">
                                            #{project.id}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h3 className="text-lg font-black text-white truncate">{project.name}</h3>
                                                <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${project.has_active_push ? 'bg-blue-500/10 text-blue-400' : 'bg-green-500/10 text-green-400'}`}>
                                                    {project.has_active_push ? 'Push Active' : 'Stable'}
                                                </span>
                                            </div>
                                            <p className="text-gray-500 text-xs font-medium truncate mb-2">Owner: {project.owner?.username} | Role: {project.user_role}</p>
                                            <div className="flex gap-6">
                                                ...
                                                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-600">
                                                    <MdAssignment size={14} className="text-red-400" />
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-white/5 rounded-2xl text-center">
                                    <p className="text-[8px] text-gray-600 font-black uppercase mb-1">Active Pushes</p>
                                    <p className="text-xl font-black text-blue-400">{projects.filter(p => p.has_active_push).length}</p>
                                </div>
                                <div className="p-4 bg-white/5 rounded-2xl text-center">
                                    <p className="text-[8px] text-gray-600 font-black uppercase mb-1">Total Versions</p>
                                    <p className="text-xl font-black text-green-400">{projects.reduce((acc, p) => acc + p.version_count, 0)}</p>
                                </div>
                            </div>
                        </div>
                    </div>
