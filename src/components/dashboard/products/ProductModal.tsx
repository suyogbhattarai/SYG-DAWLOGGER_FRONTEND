            ...
            <div className="bg-[#111e48] md:rounded-2xl w-full max-w-5xl h-full md:h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-slideUp">
                ...
                <div className="flex border-b border-white/10 bg-[#0c1a45] overflow-x-auto shrink-0 no-scrollbar">
                    {tabs.map(tab => (
                        ...
                        >
                            {tab.label}
                            {activeTab === tab.id && (
                                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#00d2ff]" />
                            )}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar bg-[#0e1736]">
                    <form id="product-form" onSubmit={handleSubmit} className="space-y-8 max-w-4xl mx-auto">
                                    ...
                                    <FormSection title="General Information">
                                        ...
                                        <div className="mt-6">
                                            ...
                                            <textarea
                                                ...
                                                className="w-full bg-[#050b2b] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#00d2ff] focus:outline-none transition resize-none text-sm"
                            ...
                            <div className="animate-fadeIn space-y-6">
                                <div className="bg-[#0a1642] p-6 rounded-2xl border border-white/5">
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <span className="text-xs bg-[#00d2ff]/10 text-[#00d2ff] px-3 py-1 rounded-full font-medium">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                        {materials.map((mat: any) => (
                                            ...
                                            >
                                                ...
                                                />
                                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${selectedMaterials.includes(mat.id) ? 'border-[#00d2ff] bg-[#00d2ff]' : 'border-gray-600 group-hover:border-gray-400'}`}>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full bg-[#050b2b] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#00d2ff] focus:outline-none transition text-sm"
                                                        placeholder="daw, version control, stem management, plugins..."
                                                    />
                                                </div>
                                                <div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="sticky top-0">
                                            ...
                                            <div className="mt-8 p-6 bg-[#00d2ff]/5 rounded-2xl border border-[#00d2ff]/10">
                                                <h4 className="flex items-center gap-2 text-[#00d2ff] text-sm font-bold mb-3">
                                                    <MdInfo /> SEO Pro-Tip
                                                </h4>
                        ...
                        {activeTab === 'images' && (
                            <div className="animate-fadeIn space-y-6">
                                {/* Upload Area */}
                                <div className="relative border-2 border-dashed border-white/10 rounded-2xl p-8 transition hover:border-[#00d2ff]/50 hover:bg-[#00d2ff]/5 group text-center cursor-pointer bg-[#0a1642]">
                                    <input
                                        type="file"
                                        multiple
                                        accept="image/*"
                                    ...
                                    />
                                    <div className="flex flex-col items-center gap-3 text-gray-400 group-hover:text-[#00d2ff] transition">
                                        <div className="bg-white/5 p-4 rounded-full group-hover:bg-[#00d2ff]/10 transition">
                ...
                <div className="p-4 md:p-6 border-t border-white/10 bg-[#0a1642] flex justify-between md:justify-end gap-3 shrink-0">
                    ...
                    <button
                        ...
                        form="product-form"
                        disabled={submitting}
                        className="flex-1 md:flex-none px-8 py-3 rounded-xl bg-[#00d2ff] hover:bg-[#e03229] text-white font-bold shadow-lg shadow-red-900/20 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
                    >
...
const InputGroup = ({ label, type = 'text', value, onChange, required, placeholder }: any) => (
    <div>
        ...
        <input
            ...
            required={required}
            className="w-full bg-[#050b2b] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#00d2ff] focus:outline-none transition text-sm font-medium placeholder-gray-600"
    ...
    <div>
        ...
        <div className="relative">
            <select
                ...
                onChange={e => onChange(e.target.value)}
                required={required}
                className="w-full bg-[#050b2b] border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#00d2ff] focus:outline-none transition text-sm appearance-none cursor-pointer"
            >
...
const Toggle = ({ label, checked, onChange }: any) => (
    <label className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all duration-200 ${checked ? 'bg-[#00d2ff]/10 border-[#00d2ff]/30' : 'bg-[#050b2b] border-white/5 hover:border-white/10'}`}>
        <span className={`text-sm font-medium ${checked ? 'text-white' : 'text-gray-400'}`}>{label}</span>
        <div className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${checked ? 'bg-[#00d2ff]' : 'bg-gray-700'}`}>
...
const ImageItem = ({ src, isPrimary, onSetPrimary, onRemove, altText, onAltChange, isExisting }: any) => (
    <div className="flex gap-4 p-3 bg-[#0a1642] rounded-xl border border-white/5 items-center group hover:border-white/20 transition">
        <div className="relative w-20 h-20 shrink-0">
            ...
            {isPrimary && (
                <div className="absolute top-1 left-1 bg-[#00d2ff] text-white text-[10px] uppercase font-bold px-2 py-0.5 rounded shadow-sm">Primary</div>
            )}
        </div>
        ...
        <div className="flex-1 min-w-0">
            {!isExisting ? (
                <input
                    ...
                    className="w-full bg-transparent border-b border-white/10 pb-1 text-sm text-white focus:border-[#00d2ff] focus:outline-none mb-2 placeholder-gray-600"
                />
            ) : (
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className={`text-xs font-medium px-3 py-1.5 rounded-lg transition ${isPrimary ? 'bg-[#00d2ff]/20 text-[#00d2ff] cursor-default' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}
                >
