...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, allowedRoles }) => {
    ...
    useEffect(() => {
        ...
        if (!isAuthenticated) {
            console.log('🛡️ ProtectedRoute: Access denied (Not authenticated), redirecting...');
            router.replace('/login');
        } else if (allowedRoles && user && !allowedRoles.includes(user.role)) {
            console.log(`🛡️ ProtectedRoute: Role ${user.role} not in allowed roles:`, allowedRoles);
            ...
            console.log('🛡️ ProtectedRoute: Access granted for role:', user?.role);
            setIsAuthorized(true);
        }
    }, [isAuthenticated, loading, router, user, allowedRoles, mounted]);

    // Show loading spinner while checking auth state or if Redux is loading or if not mounted
